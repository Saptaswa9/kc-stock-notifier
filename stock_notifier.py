...
NEW products are listed (e.g. new Hot Wheels and Majorette cars).

How it works
------------
Shopify exposes a public JSON feed for every collection at
    /collections/<handle>/products.json
This script reads that feed, remembers which product IDs it has already seen
(in a small JSON file on disk), and on each run reports only the IDs it has
never seen before. The first run just records a baseline silently so you don't
get spammed with the entire existing catalogue.
...
USER_AGENT = "Mozilla/5.0 (compatible; KCStockNotifier/1.0; personal use)"
...
def run_once(notify=True):
    ...
    for name, handle in COLLECTIONS.items():
        ...
        state[handle] = current_ids
        if first_run:
            log(f"[baseline] {name}: tracking {len(current_ids)} products")
        else:
            log(f"{name}: {fresh} new")

    save_state(state)

    if first_run:
        log("Baseline established. New listings from now on will trigger alerts.")
        return []

    # A product can appear in more than one watched collection; dedupe by id.
    unique = list({it["id"]: it for it in new_items}.values())
    if unique and notify:
        send_notifications(unique)
    elif not unique:
        log("No new listings.")
    return unique
...
def build_message(items):
    ...
    header = f"{len(items)} new listing(s) on Krazy Caterpillar:"
    return header + "\n\n" + "\n".join(lines)


def notify_discord(text):
    webhook = os.environ["DISCORD_WEBHOOK_URL"]
    # Discord caps message content at 2000 chars; chunk if needed.
...
def notify_email(text):
    ...
    msg["Subject"] = "New Krazy Caterpillar listings"
    msg["From"] = sender
    msg["To"] = recipient
